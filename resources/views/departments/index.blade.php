                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-750">
                            ...
                            <td class="px-6 py-3">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800 dark:bg-indigo-900 dark:text-indigo-200">
                                    {{ $department->funcionarios_count }}
                                </span>
                            </td>
                            <td class="px-6 py-3 text-right space-x-3">
                                <a href="{{ route('departments.show', $department) }}" class="text-blue-600 hover:underline text-xs">Ver</a>
                                @if(auth()->user()->isAdmin())
                                <a href="{{ route('departments.edit', $department) }}" class="text-indigo-600 hover:underline text-xs">Editar</a>
                                <form action="{{ route('departments.destroy', $department) }}" method="POST" class="inline"
                                      onsubmit="return confirm('Excluir o departamento {{ $department->nome }}? Os funcionários serão desvinculados.')">
                                    @csrf @method('DELETE')
