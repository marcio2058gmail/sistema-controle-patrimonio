    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
                <form action="{{ route('funcionarios.update', $funcionario) }}" method="POST" class="space-y-5">
                    @csrf @method('PATCH')

                    <div>
                        <x-input-label for="nome" value="Nome *" />
                        <x-text-input id="nome" name="nome" type="text"
                            class="mt-1 block w-full" :value="old('nome', $funcionario->nome)" required />
                    ...
                    <div>
                        ...
                        <x-text-input id="email" name="email" type="email"
                            class="mt-1 block w-full" :value="old('email', $funcionario->email)" required />
                    ...
                    <div>
                        ...
                        <x-text-input id="cargo" name="cargo" type="text"
                            class="mt-1 block w-full" :value="old('cargo', $funcionario->cargo)" />
                    ...
                    <div>
                        ...
                        <select id="departamento_id" name="departamento_id"
                            ...
                            @foreach($departamentos as $dep)
                                <option value="{{ $dep->id }}" {{ old('departamento_id', $funcionario->departamento_id) == $dep->id ? 'selected' : '' }}>
                                    {{ $dep->nome }}
                                </option>
                            @endforeach
                        </select>
