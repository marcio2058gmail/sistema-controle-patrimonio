...
<head>
    ...
    <title>Termo de Responsabilidade #{{ $responsabilidade->id }}</title>
    <style>
...
<div class="page">
    ...
    <div class="header">
        ...
        <p>Documento de Controle de Bens — Registro #{{ $responsabilidade->id }}</p>
    </div>
    ...
    <table class="info">
        <tr>
            ...
            <td>{{ $responsabilidade->funcionario->nome }}</td>
            <td class="label">Cargo</td>
            <td>{{ $responsabilidade->funcionario->cargo ?? '—' }}</td>
        </tr>
        <tr>
            <td class="label">E-mail</td>
            <td colspan="3">{{ $responsabilidade->funcionario->email }}</td>
        </tr>
    </table>
    ...
    <table class="info">
        <tr>
            ...
            <td>{{ $responsabilidade->patrimonio->codigo_patrimonio }}</td>
            <td class="label">Status</td>
            <td>
                @if(! $responsabilidade->data_devolucao)
        ...
        <tr>
            ...
            <td colspan="3">{{ $responsabilidade->patrimonio->descricao }}</td>
        </tr>
        <tr>
            <td class="label">Modelo</td>
            <td>{{ $responsabilidade->patrimonio->modelo ?? '—' }}</td>
            <td class="label">Número de Série</td>
            <td>{{ $responsabilidade->patrimonio->numero_serie ?? '—' }}</td>
        </tr>
    </table>
    ...
    <table class="info">
        <tr>
            ...
            <td>{{ $responsabilidade->data_entrega->format('d/m/Y') }}</td>
            <td class="label">Data de Devolução</td>
            <td>{{ $responsabilidade->data_devolucao?->format('d/m/Y') ?? 'Em aberto' }}</td>
        </tr>
        <tr>
            <td class="label">Assinado Fisicamente</td>
            <td colspan="3">{{ $responsabilidade->assinado ? 'Sim' : 'Não' }}</td>
        </tr>
    </table>

    <div class="section-title">Declaração de Responsabilidade</div>
    <div class="termo-box">{{ $responsabilidade->termo_responsabilidade }}</div>

    <div class="assinaturas">
        <div class="assinatura-bloco">
            <div class="assinatura-linha"></div>
            <div class="assinatura-nome">
                {{ $responsabilidade->funcionario->nome }}<br>
