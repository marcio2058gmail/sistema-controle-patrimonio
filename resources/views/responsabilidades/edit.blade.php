<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            Editar Responsabilidade #{{ $responsabilidade->id }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-6 mb-4">
                <p class="text-sm text-gray-500">
                    Funcionário: <strong class="text-gray-800 dark:text-gray-200">{{ $responsabilidade->funcionario->nome }}</strong>
                    &nbsp;|&nbsp;
                    Patrimônio: <strong class="text-gray-800 dark:text-gray-200 font-mono">{{ $responsabilidade->patrimonio->codigo_patrimonio }}</strong>
                </p>
            </div>

            <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
                <form action="{{ route('responsabilidades.update', $responsabilidade) }}" method="POST" class="space-y-5">
                    @csrf @method('PATCH')
                    ...
                    <div>
                        ...
                        <x-text-input id="data_devolucao" name="data_devolucao" type="date"
                            ...
                            :value="old('data_devolucao', $responsabilidade->data_devolucao?->toDateString())" />
                    ...
                    <div>
                        ...
                        <textarea id="termo_responsabilidade" name="termo_responsabilidade" rows="6"
                            class="mt-1 block w-full border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-300 rounded-md shadow-sm focus:ring focus:ring-indigo-300">{{ old('termo_responsabilidade', $responsabilidade->termo_responsabilidade) }}</textarea>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <input id="assinado" name="assinado" type="checkbox" value="1"
                            {{ old('assinado', $responsabilidade->assinado) ? 'checked' : '' }}
