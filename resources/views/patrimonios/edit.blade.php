<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            Editar Patrimônio — {{ $patrimonio->codigo_patrimonio }}
        </h2>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-6">
                <form action="{{ route('patrimonios.update', $patrimonio) }}" method="POST" class="space-y-5">
                    @csrf @method('PATCH')
                    ...
                    <div>
                        ...
                        <x-text-input id="codigo_patrimonio" name="codigo_patrimonio" type="text"
                            class="mt-1 block w-full" :value="old('codigo_patrimonio', $patrimonio->codigo_patrimonio)" required />
                    ...
                    <div>
                        ...
                        <x-text-input id="descricao" name="descricao" type="text"
                            class="mt-1 block w-full" :value="old('descricao', $patrimonio->descricao)" required />
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="modelo" name="modelo" type="text"
                                class="mt-1 block w-full" :value="old('modelo', $patrimonio->modelo)" />
                        </div>
                        <div>
                            <x-input-label for="numero_serie" value="Número de Série" />
                            <x-text-input id="numero_serie" name="numero_serie" type="text"
                                class="mt-1 block w-full" :value="old('numero_serie', $patrimonio->numero_serie)" />
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <select id="status" name="status"
                            ...
                            @foreach($statusLabels as $value => $label)
                                <option value="{{ $value }}" {{ old('status', $patrimonio->status) === $value ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
